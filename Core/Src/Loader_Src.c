...
Write(uint32_t Address, uint32_t Size, uint8_t* buffer) {
    ...
    if (QSPI_W25Qxx_WriteBuffer((uint8_t*) buffer, (Address & (0x0fffffff)), Size) != QSPI_W25Qxx_OK) {
        __set_PRIMASK(1); //disable interrupts
        return LOADER_FAIL;
    }
...
SectorErase(uint32_t EraseStartAddress, uint32_t EraseEndAddress) {
	...
	while (EraseEndAddress >= EraseStartAddress) {
		...
		if (QSPI_W25Qxx_BlockErase_64K(BlockAddr) != QSPI_W25Qxx_OK)  // ������Ӧ����
		{
			__set_PRIMASK(1); //disable interrupts
			return LOADER_FAIL;
		}
...
MassErase(void) {
    ...
    if (QSPI_W25Qxx_ChipErase() != HAL_OK) {
        __set_PRIMASK(1); //disable interrupts
        return LOADER_FAIL;
    }
